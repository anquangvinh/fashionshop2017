                        ...
                        <c:forEach items="${cartList}" var="item">
                            <tr>
                                ...
                                <td style="width: 100px;" align="center">
                                    <input type="number"
                                           id="${item.getProduct().productID}-${item.getSizesByColor().getSizeID()}-${item.getSizesByColor().getColor().getColorID()}"
                                           onchange="changeQuantity(this.name, this.value)" 
                                           name="${item.getProduct().productID}-${item.getSizesByColor().getSizeID()}-${item.getSizesByColor().getColor().getColorID()}" 
                                           value="${item.quantity}"
                                           style="width: 70px;"
                                           class="form-control"
                                           min="1"/>
...
<script type="text/javascript">
    function changeQuantity(name, value) {
        if (value < 1) {
            $('#error-shoppingcart').html("<div class=\"alert alert-danger\"><strong>QUANTITY MUST EQUAL OR GREATER THAN 1</strong></div>");
            $('body,html').animate({
                scrollTop: 0
            }, 500);
        } else if (Math.floor(value) != value) {
            $('#error-shoppingcart').html("<div class=\"alert alert-danger\"><strong>QUANTITY MUST NOT BE DECIMAL</strong></div>");
            $('body,html').animate({
                scrollTop: 0
            }, 500);
        } else {
            window.location = 'orders/updatecart/' + name + '/' + value + '.html';
        }
    }
    ;
</script>
